...
func NewCertRevokeOperation(cert, priv []byte) BuiltinOperation {
	msg := "revoke"
	sign := []byte(msg)
	if priv != nil {
		key, err := ParsePriv(priv)
		if err != nil {
			return newCertOperation(CertRevoke, string(cert), msg, hex.EncodeToString(sign))
		}
		sign, _ = key.Sign([]byte(msg))
	}
	return newCertOperation(CertRevoke, string(cert), msg, hex.EncodeToString(sign))
}
