...
func TestBVMCert(t *testing.T) {
	...
	priv := []byte("-----BEGIN EC PRIVATE KEY-----\n" +
		"MHQCAQEEIHJjxPVY8G1tofQtLixfAsvx2YYsqohXcR0yozJmoL0zoAcGBSuBBAAK\n" +
		"oUQDQgAEEjd7A0rbW8c048bpTdeYomiFI03PvZAoPFSlG3R8IkfBoZh9K9jhyeqR\n" +
		"uWJyV9yRROkKQVJGvhyo3o9REIXORQ==\n" +
		"-----END EC PRIVATE KEY-----")
	key, err := account.NewAccountFromAccountJSON(accountJsons[0], pwd)
	assert.Nil(t, err)
	operation := bvm.NewCertRevokeOperation(ecert, priv)
	payload := bvm.EncodeOperation(operation)
	tx := NewTransaction(key.GetAddress().Hex()).Invoke(operation.Address(), payload).VMType(BVM)
	tx.Sign(key)
	re, err := rpc.InvokeContract(tx)
