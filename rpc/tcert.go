...
func NewTCertManager2(vip *viper.Viper) *TCertManager {
	if !vip.GetBool(common.PrivacySendTcert) {
		return nil
	}

	sdkcertPath := vip.GetString(common.PrivacySDKcertPath)
	logger.Debugf("[CONFIG]: sdkcertPath = %v", sdkcertPath)

	sdkcertPriPath := vip.GetString(common.PrivacySDKcertPrivPath)
	logger.Debugf("[CONFIG]: sdkcertPriPath = %v", sdkcertPriPath)

	uniquePubPath := vip.GetString(common.PrivacyUniquePubPath)
	logger.Debugf("[CONFIG]: uniquePubPath = %v", uniquePubPath)

	uniquePrivPath := vip.GetString(common.PrivacyUniquePrivPath)
