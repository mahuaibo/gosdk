...
func newHTTPRequestManager2(vip *viper.Viper, txVersion string) (hrm *httpRequestManager) {
	var (
		namespace string
		urls      []string
		rpcPorts  []string
	...
	if sendTcert && !vip.GetBool(common.PrivacyCfca) && !isFlato(txVersion) {
		...
		for _, node := range nodes {
			tcert, err := httpRequestManager.getTCert2(node.url)
			if err != nil {
...
func newHTTPClient2(vip *viper.Viper) *http.Client {
	...
	if vip.GetBool(common.SecurityHttps) {
		...
		tlscaPath := vip.GetString(common.SecurityTlsca)
		tlspeerCertPath := vip.GetString(common.SecurityTlspeerCert)
		tlspeerCertPrivPath := vip.GetString(common.SecurityTlspeerPriv)

		caCrt := []byte(tlscaPath)

		pool.AppendCertsFromPEM(caCrt)

		cliCrt, err := tls.X509KeyPair([]byte(tlspeerCertPath), []byte(tlspeerCertPrivPath))
		if err != nil {
...
func (hrm *httpRequestManager) getTCert2(url string) (string, StdError) {
	rawReq := &JSONRequest{
	...
	uniqPub := []byte(hrm.tcm.uniquePubPath)
	//if sysErr != nil {
